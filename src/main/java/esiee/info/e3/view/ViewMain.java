...
public class ViewMain implements IView {
  ...
  public void navigateTo(String name, boolean withLoading) {
    ...
    @Override
    public void triggerJokerReplacement(JokerType newJoker, Runnable onCancel, java.util.function.Consumer<JokerType> onConfirm) {
        if (Objects.requireNonNull(this.currentPage) instanceof GamePage gamePage) {
            gamePage.triggerJokerSelectionForReplacement(newJoker, onCancel, onConfirm);
        }
