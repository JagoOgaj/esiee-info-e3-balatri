...
public final class ScoreCalculator implements IScoreCalculator {
  ...
  public long calculateScore(
    ...
    if (state == null || activeJokers == null || activeJokers.isEmpty()) {
      return (long) Math.max(0, chips) * Math.max(0, multiplier);
    }

    var scoreResult = new ScoreResult(chips, multiplier);
    var context =
        new JokerContext(combo, activeCards, cards, level, constraint, state, isPreview);
  ...
  private List<Card> getActiveCards(Combination combo, List<Card> cards) {
    ...
    return switch (combo) {
      case COMBINATION_SQUARE ->
          cards.stream().filter(c -> rankCounts.getOrDefault(c.rank(), 0L) >= 4L).toList();
      case COMBINATION_THREE_OF_KIND ->
          cards.stream().filter(c -> rankCounts.getOrDefault(c.rank(), 0L) >= 3L).toList();
      case COMBINATION_DOUBLE_PAIR, COMBINATION_PAIR ->
          cards.stream().filter(c -> rankCounts.getOrDefault(c.rank(), 0L) >= 2L).toList();
      case COMBINATION_HIGH_MAP -> {
        var highestCard =
            cards.stream().max(Comparator.comparingInt(c -> c.rank().getValue())).orElse(null);
        yield List.of(highestCard);
      }
      default -> cards;
    };
  }
