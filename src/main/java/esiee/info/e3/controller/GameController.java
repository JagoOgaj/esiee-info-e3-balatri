...
public record GameController(IGameModel model, IView view, ISaveManager saveManager)
    ...
    public void handleRerollShop() {
        try {
            model.executeRerollShopAction();
        } catch (IllegalStateException e) {
            if (ExceptionConstant.ERROR_NO_MONEY_REROLL.name().equals(e.getMessage())) {
                view.showOverlay(OverlayType.ERROR, TextConstant.TEXT_SHOP_ERROR_NO_MONEY_REROLL.getText(), null);
            }
        }
    }
    ...
    public void handleBuyShopItem(int index) {
        try {
            model.executeBuyShopItemAction(index);
        } catch (IllegalStateException e) {
            String msg = e.getMessage();
            if (ExceptionConstant.ERROR_NO_MONEY_BUY.name().equals(msg)) {
                view.showOverlay(OverlayType.ERROR, TextConstant.TEXT_SHOP_ERROR_NO_MONEY_BUY.getText(), null);
            } else if (ExceptionConstant.ERROR_INVENTORY_FULL.name().equals(msg)) {
                view.showOverlay(OverlayType.ERROR, TextConstant.TEXT_SHOP_ERROR_INVENTORY_FULL.getText(), null);
            }
        }
    }
